...
func Shuffle(group abstract.Group, g, h abstract.Point, X, Y []abstract.Point,
	rand cipher.Stream) (XX, YY []abstract.Point, P proof.Prover, XXX, YYY []abstract.Point) {
	...
	tmpXbar := make([]abstract.Point, k)
	tmpYbar := make([]abstract.Point, k)
	for i := 0; i < k; i++ {
		Xbar[i] = ps.grp.Point().Mul(g, beta[pi[i]])
		tmpXbar[i] = ps.grp.Point().Mul(g, beta[pi[i]])
		Xbar[i].Add(Xbar[i], X[pi[i]])
		Ybar[i] = ps.grp.Point().Mul(h, beta[pi[i]])
		tmpYbar[i] = ps.grp.Point().Mul(h, beta[pi[i]])
	...
	return Xbar, Ybar, prover, tmpXbar, tmpYbar
}
